...
pub fn collect_files(root_path: &Path) -> Result<Vec<PathBuf>, String> {
    if !root_path.exists() {
        return Err(format!(
    ...
    visit_directory(root_path, &mut files)?;
    Ok(files)
}

/// Visits a directory depth-first and collects regular file paths.
fn visit_directory(current_path: &Path, files: &mut Vec<PathBuf>) -> Result<(), String> {
    ...
    for entry_result in entries {
        match entry_result {
            Ok(entry) => {
                ...
                if path.is_dir() {
                    if let Err(error) = visit_directory(&path, files) {
                        log_warning(
                            "directory_skipped",
                            &[
